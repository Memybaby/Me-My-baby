...
const COUNTRIES = [
  { code: "CA", currency: "CAD", label: { fr: "Canada", en: "Canada", es: "Canadá" } },
  { code: "US", currency: "USD", label: { fr: "États-Unis", en: "United States", es: "Estados Unidos" } },
  { code: "FR", currency: "EUR", label: { fr: "France", en: "France", es: "Francia" } },
  { code: "BE", currency: "EUR", label: { fr: "Belgique", en: "Belgium", es: "Bélgica" } },
  { code: "CH", currency: "EUR", label: { fr: "Suisse", en: "Switzerland", es: "Suiza" } },
  { code: "ES", currency: "EUR", label: { fr: "Espagne", en: "Spain", es: "España" } },
  { code: "MX", currency: "USD", label: { fr: "Mexique", en: "Mexico", es: "México" } },
  { code: "OTHER", currency: "USD", label: { fr: "Autre pays", en: "Other country", es: "Otro país" } },
];
...
/* ---------------- NAV CONFIG ---------------- */
/* ---------------- PROFILE ---------------- */
function ProfileSection({ lang, setLang, children, setChildren, userProfile, onLogout, session, justSignedUp, onOnboardingDone, standalone }) {
  ...
  const handleChildrenCountChange = (value) => {
    ...
    setChildDrafts((prev) => Array.from({ length: count }, (_, i) => prev[i] || { name: "", ageValue: "", ageUnit: "years", photo: null, photoFile: null }));
  };
  const updateChildDraft = (i, field, value) => {
    setChildDrafts((prev) => prev.map((d, idx) => (idx === i ? { ...d, [field]: value } : d)));
  };
  const handleChildDraftPhoto = (i, e) => {
    const file = e.target.files?.[0];
    if (!file) return;
    const reader = new FileReader();
    reader.onload = () => updateChildDraft(i, "photo", reader.result);
    reader.readAsDataURL(file);
    updateChildDraft(i, "photoFile", file);
  };
    ...
    if (hasSession) {
      ...
      try {
        await supabaseUpdateProfile(session.user.id, {
          first_name: firstName || "", last_name: rest.join(" "), mother_age: form.age || null,
          username: form.username || null, due_date: form.dueDate || null, children: form.children,
          country: form.country || null, language: form.language || "fr",
        }, session.access_token);
      } catch (e) { /* on continue même si la mise à jour du profil échoue */ }
    }
    if (form.language && form.language !== lang && setLang) setLang(form.language); // applique la nouvelle langue immédiatement
    // 2. Création des fiches enfant remplies
    const filledDrafts = childDrafts.filter((d) => d.name.trim());
    for (const draft of filledDrafts) {
      const birthdate = draft.ageValue ? computeBirthdateFromAge(draft.ageValue, draft.ageUnit) : "";
      if (hasSession) {
        try {
          let photoUrl = null;
        ...
        } catch (e) { /* on ignore l'échec d'un enfant en particulier pour ne pas bloquer les autres */ }
      } else {
  ...
  const L = lang === "fr"
    ? { title: "Mon profil", subtitle: "Vos informations personnelles.", name: "Nom complet", email: "Courriel",
        ...
        age: "Votre âge", children: "Nombre d'enfants", due: "Date prévue d'accouchement (si applicable)", edit: "Modifier", save: "Enregistrer",
        ...
        childName: (i) => `Prénom de l'enfant ${i + 1}`, childAge: "Âge", years: "ans", months: "mois",
    ...
    ? { title: "Mi perfil", subtitle: "Tu información personal.", name: "Nombre completo", email: "Correo electrónico",
        ...
        age: "Tu edad", children: "Número de hijos", due: "Fecha probable de parto (si aplica)", edit: "Editar", save: "Guardar",
        ...
        childName: (i) => `Nombre del hijo/a ${i + 1}`, childAge: "Edad", years: "años", months: "meses",
    ...
    : { title: "My profile", subtitle: "Your personal information.", name: "Full name", email: "Email",
        ...
        age: "Your age", children: "Number of children", due: "Expected due date (if applicable)", edit: "Edit", save: "Save",
  ...
  return (
    <div>
      ...
      <Card style={{ marginBottom: 8 }}>
        {editing ? (
          <div>
            ...
            <div style={{ display: "flex", gap: 14, flexWrap: "wrap" }}>
              <label style={{ ...labelStyle, flex: 1, minWidth: 140 }}>{L.age}
                <input type="number" min={13} max={60} style={inputStyle} value={form.age} onChange={(e) => setForm({ ...form, age: e.target.value })} />
              </label>
                  ...
                  <div key={i} style={{ background: COLORS.cream, borderRadius: 14, padding: 14, display: "flex", gap: 12, alignItems: "flex-start" }}>
                    ...
                    <div style={{ flex: 1, display: "flex", flexDirection: "column", gap: 8, minWidth: 0 }}>
                      ...
                      <div style={{ display: "flex", gap: 8 }}>
                        <input
                          type="number" min={0} placeholder={L.childAge} value={draft.ageValue}
                          onChange={(e) => updateChildDraft(i, "ageValue", e.target.value)}
                          style={{ width: 80, padding: "9px 10px", borderRadius: 10, border: `1px solid ${COLORS.line}`, fontSize: 13.5, boxSizing: "border-box" }}
                        />
                        <div style={{ display: "flex", borderRadius: 10, overflow: "hidden", border: `1px solid ${COLORS.line}` }}>
                          {["years", "months"].map((u) => (
                            <button
                              key={u} type="button" onClick={() => updateChildDraft(i, "ageUnit", u)}
                              style={{
                                padding: "8px 12px", border: "none", fontSize: 12.5, fontWeight: 700, cursor: "pointer",
                                background: draft.ageUnit === u ? COLORS.pink : "#fff",
                                color: draft.ageUnit === u ? "#fff" : COLORS.text,
                              }}
                            >{u === "years" ? L.years : L.months}</button>
                          ))}
                        </div>
                      </div>
                    </div>
      ...
      {!standalone && <p style={{ fontSize: 11.5, color: COLORS.muted, marginTop: 4, marginBottom: 24, textAlign: "center", fontStyle: "italic" }}>{L.demo}</p>}
...
function ProfileGate({ lang, onComplete, onSkip }) {
  ...
  const handleSubmit = async () => {
    if (!(form.firstName.trim() && form.email.trim() && form.password.length >= 8)) return;
    setLoading(true);
    setErrorMsg("");
  ...
  return (
    <div style={{
      ...
      <div style={{ width: "100%", maxWidth: 440 }}>
        ...
        {step === "form" && mode === "signup" && <p style={{ fontSize: 12, color: COLORS.muted, textAlign: "center", marginTop: 14, lineHeight: 1.5 }}>{L.note}</p>}
        {step === "form" && (
        <button
...
export default function App() {
  ...
  const renderContent = () => {
    switch (active) {
      ...
      case "profil": return <ProfileSection lang={lang} setLang={setLang} children={children} setChildren={setChildren} userProfile={userProfile} onLogout={logout} session={session} justSignedUp={justSignedUp} onOnboardingDone={() => setJustSignedUp(false)} />;
