...
// TEST DE DIAGNOSTIC : cette version ne fait plus AUCUN traitement de l'image dans le navigateur
// (ni createImageBitmap, ni canvas, ni redimensionnement). Elle envoie le fichier tel quel et
// génère un aperçu ultra-léger avec URL.createObjectURL (qui ne décode jamais l'image en pixels,
// contrairement à FileReader.readAsDataURL ou à un dessin sur canvas). Le but est d'isoler si les
// plantages mémoire venaient de ce traitement d'image spécifique, ou d'une autre cause.
async function resizeImageForUpload(file, maxDim = 640, quality = 0.75) {
  ...
  try {
    const dataUrl = URL.createObjectURL(file); // aucun décodage de pixels, juste une référence légère
    return { dataUrl, file };
  } catch (e) {
