...
function ProfileSection({ lang, setLang, children, setChildren, userProfile, onLogout, session, justSignedUp, onOnboardingDone, standalone, isMember, onBecomeMember }) {
  ...
  const handleUpcomingChildPhoto = async (e) => {
    ...
    setUpcomingChild((u) => ({ ...u, photo: dataUrl, photoFile: resizedFile }));
    if (hasSession) {
      try {
      ...
      } catch (e) { /* on retentera au moment d'Enregistrer */ }
    }
  };
  ...
  const handleChildDraftPhoto = async (i, e) => {
    ...
    if (!file) return;
    const { dataUrl, file: resizedFile } = await resizeImageForUpload(file);
    updateChildDraft(i, "photo", dataUrl); // aperçu immédiat pendant l'envoi
    updateChildDraft(i, "photoFile", resizedFile);
    if (hasSession) {
      try {
        const url = await supabaseUploadPhoto(resizedFile, session.user.id, session.access_token);
        // On remplace l'aperçu local (lourd en mémoire) par le lien distant déjà envoyé, et on
        // vide la référence au fichier : ainsi une seule photo à la fois reste "lourde" en mémoire,
        // au lieu d'accumuler les 2-3 photos des enfants jusqu'au clic final sur "Continuer".
        updateChildDraft(i, "photo", url);
        updateChildDraft(i, "photoFile", null);
        updateChildDraft(i, "photoUploaded", true);
      } catch (e) { /* échec d'envoi : on retente l'envoi au moment d'Enregistrer, avec le fichier gardé en mémoire */ }
    }
  };
  ...
  const handleSave = async () => {
    ...
    // 2. Création des fiches enfant remplies (nés)
    ...
    for (const draft of filledDrafts) {
      ...
      if (hasSession) {
        try {
          let photoUrl = draft.photoUploaded ? draft.photo : null; // déjà envoyée immédiatement lors de la sélection
          if (!photoUrl && draft.photoFile) photoUrl = await supabaseUploadPhoto(draft.photoFile, session.user.id, session.access_token);
    ...
    // 3. Création de la fiche "enfant à venir" (date de naissance = DPA tant que non modifiée), si un prénom y a été saisi
    let upcomingSaved = true;
    if (upcomingChild.name.trim()) {
      ...
      if (hasSession) {
        try {
          ...
          const row = await supabaseAddChild({ name: upcomingChild.name.trim(), birthdate, photo: photoUrl }, session.user.id, session.access_token);
          setChildren((c) => [...c, mapChildRow(row)]);
        } catch (e) { hadError = true; upcomingSaved = false; }
      } else {
        setChildren((c) => [...c, { name: upcomingChild.name.trim(), birthdate, photo: upcomingChild.photo, id: `local-${Date.now()}-${Math.random()}` }]);
      }
    }
    if (upcomingSaved) setUpcomingChild({ name: "", birthdate: "", photo: null, photoFile: null, photoUploaded: false, birthdateTouched: false });
  ...
  const L = lang === "fr"
    ? { title: "Mon profil", subtitle: "Vos informations personnelles.", name: "Nom complet", email: "Courriel",
        ...
        upcomingDueNote: "Basée sur votre date prévue d'accouchement — modifiable une fois bébé né." }
    : lang === "es"
    ? { title: "Mi perfil", subtitle: "Tu información personal.", name: "Nombre completo", email: "Correo electrónico",
        ...
        upcomingDueNote: "Basada en tu fecha probable de parto — se puede actualizar cuando nazca el bebé." }
    : { title: "My profile", subtitle: "Your personal information.", name: "Full name", email: "Email",
        ...
        upcomingDueNote: "Based on your expected due date — you can update this once baby is born." };
      ...
      <Card style={{ marginBottom: 8 }}>
        {editing ? (
          <div>
            ...
            <label style={{ ...labelStyle, maxWidth: 260 }}>{L.children}
