...
async function supabaseUpdateProfile(userId, updates, accessToken) {
  ...
  if (!res.ok) {
    ...
    throw new Error(`Impossible de mettre à jour le profil (${res.status}: ${errText || "réponse vide"})`);
  }
}
...
// CONFIRMÉ par test réel : le plantage mémoire vient bien de createImageBitmap/canvas eux-mêmes sur
// ce téléphone précis (probablement un bug ou une limite du décodage accéléré GPU de ce navigateur),
// pas de canvas.toDataURL() spécifiquement comme on le pensait à l'étape précédente. La version qui
// a fonctionné sans plantage est celle qui n'utilise AUCUNE de ces API : on envoie le fichier original
// tel quel et on génère un aperçu ultra-léger avec URL.createObjectURL (aucun décodage de pixels par
// notre code — seul le navigateur décode, au moment de l'affichage, de façon optimisée en interne).
// Contrepartie : les photos ne sont plus compressées avant l'envoi (fichiers plus lourds, upload un
// peu plus lent), mais c'est nettement préférable à un plantage qui perd les données de la personne.
...
function ProfileSection({ lang, setLang, children, setChildren, userProfile, onLogout, session, justSignedUp, onOnboardingDone, standalone, isMember, onBecomeMember }) {
  ...
  const handleSave = async () => {
    ...
    if (hasSession && photoFile) {
      ...
      } catch (e) { hadError = true; errorDetail = e?.message || String(e); }
    }
    ...
    if (hasSession && !hadError) {
      ...
      try {
        await supabaseUpdateProfile(session.user.id, {
          ...
          due_date: form.dueDate || null, children: String(children.length),
    ...
    setSaving(false);
    if (hadError) {
      setErrorMsg(`${L.errorSave} [${errorDetail}]`);
      ...
      <Card style={{ marginBottom: 8 }}>
        {editing ? (
          <div>
            ...
            {(() => {
              const target = Number(expectedChildrenCount);
              // La carte d'ajout reste visible tant que le nombre d'enfants déjà ajoutés n'a pas
              // atteint le nombre indiqué par la personne (éviter d'en proposer plus que demandé).
              return target > 0 && children.length < target;
            })() && (
