...
// peuvent faire planter la page en mémoire dès le CHARGEMENT si on les décode à pleine résolution.
// IMPORTANT : contrairement à une version précédente de cette fonction, on n'essaie JAMAIS de décoder
// l'image nous-mêmes à pleine résolution (via <img> + canvas) en cas d'échec du redimensionnement natif —
// cette méthode de secours était elle-même la cause probable des plantages mémoire sur les photos très
// haute résolution. Si le redimensionnement économe en mémoire n'est pas possible, on envoie simplement
// le fichier original tel quel (le navigateur peut le transmettre au serveur sans jamais le décoder en pixels).
async function resizeImageForUpload(file, maxDim = 640, quality = 0.75) {
  ...
  if (typeof createImageBitmap !== "function") return { dataUrl: null, file };

  try {
    // Le navigateur redimensionne PENDANT le décodage (resizeWidth), sans jamais allouer
    // la pleine résolution d'origine en mémoire — c'est ce qui rend cette méthode sûre.
    ...
    // Échec du redimensionnement sûr : on n'essaie surtout PAS la méthode de secours coûteuse en mémoire.
    // L'aperçu apparaîtra une fois la photo téléversée (via l'URL distante), pas avant.
