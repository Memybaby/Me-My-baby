...
function AbonnementUnified({ lang, isMember, goTo, onBecomeMember }) {
  return isMember
    ? <MySubscriptionSection lang={lang} goTo={goTo} />
    : <MembershipSection lang={lang} goTo={goTo} onBecomeMember={onBecomeMember} />;
}

function MembershipSection({ lang, goTo, onBecomeMember }) {
  const m = MEMBERSHIP[lang];
  const p = PLANS[lang][0];
  const [billing, setBilling] = useState("annual");
  const [currency, setCurrency] = useState(lang === "fr" ? "CAD" : "USD");
      ...
      <div style={{ display: "flex", justifyContent: "center", gap: 8, marginBottom: 24 }}>
        {["CAD", "USD", "EUR"].map((c) => (
          <button key={c} onClick={() => setCurrency(c)} style={{
            ...
            border: `1px solid ${currency === c ? COLORS.teal : COLORS.line}`,
            background: currency === c ? COLORS.teal : "#fff",
...
/* ---------------- NAV CONFIG ---------------- */
/* ---------------- PROFILE ---------------- */
function ProfileSection({ lang, children, setChildren, userProfile, onLogout, session }) {
  ...
  const [form, setForm] = useState({
    ...
    children: userProfile?.children ?? "1",
  ...
  const L = lang === "fr"
    ? { title: "Mon profil", subtitle: "Vos informations personnelles.", name: "Nom complet", email: "Courriel",
        children: "Nombre d'enfants", due: "Date prévue d'accouchement", edit: "Modifier", save: "Enregistrer",
    ...
    ? { title: "Mi perfil", subtitle: "Tu información personal.", name: "Nombre completo", email: "Correo electrónico",
        children: "Número de hijos", due: "Fecha probable de parto", edit: "Editar", save: "Guardar",
    ...
    : { title: "My profile", subtitle: "Your personal information.", name: "Full name", email: "Email",
        children: "Number of children", due: "Expected due date", edit: "Edit", save: "Save",
    ...
    <div>
      <div style={{
        ...
        <div style={{ position: "relative", display: "inline-block", margin: "0 auto 14px" }}>
          <div style={{
            width: 72, height: 72, borderRadius: "50%", background: COLORS.ochre, color: "#fff",
            display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "Fraunces, Georgia, serif",
            fontSize: 26, fontWeight: 700, flexShrink: 0, position: "relative", overflow: "hidden",
            border: "3px solid rgba(255,255,255,0.5)", boxShadow: "0 6px 16px rgba(0,0,0,0.15)",
          }}>
          ...
          <button
            ...
            style={{
              ...
              background: COLORS.ochre, border: "2px solid #fff", color: "#fff", cursor: "pointer",
        ...
        {editing ? (
          <div>
            ...
            <div style={{ display: "flex", gap: 10, marginTop: 6 }}>
              <button onClick={() => { setEditing(false); setSaved(true); }} style={{ background: COLORS.teal, color: "#fff", border: "none", padding: "10px 22px", borderRadius: 999, fontWeight: 700, fontSize: 14, cursor: "pointer" }}>{L.save}</button>
              <button onClick={() => setEditing(false)} style={{ background: "transparent", color: COLORS.muted, border: `1px solid ${COLORS.line}`, padding: "10px 22px", borderRadius: 999, fontWeight: 600, fontSize: 14, cursor: "pointer" }}>{L.cancel}</button>
            </div>
          </div>
        ) : (
          <div>
      ...
      <p style={{ fontSize: 11.5, color: COLORS.muted, marginTop: 4, marginBottom: 24, textAlign: "center", fontStyle: "italic" }}>{L.demo}</p>
...
function ProfileGate({ lang, onComplete, onSkip }) {
  ...
  const [form, setForm] = useState({ firstName: "", lastName: "", email: "", password: "", motherAge: "", children: "0", childrenAgesArr: [], dueDate: "", pregnancyProfile: [], newsletter: true });
  const [loginForm, setLoginForm] = useState({ email: "", password: "" });
  const [loading, setLoading] = useState(false);
  const [errorMsg, setErrorMsg] = useState("");

  // Ajuste le nombre de champs d'âge disponibles selon le nombre d'enfants saisi
  const handleChildrenChange = (value) => {
    const count = Math.max(0, Math.min(20, Number(value) || 0));
    setForm((prev) => {
      const nextAges = Array.from({ length: count }, (_, i) => prev.childrenAgesArr[i] || "");
      return { ...prev, children: value, childrenAgesArr: nextAges };
    });
  };

  const handleChildAgeChange = (index, value) => {
    if (value !== "" && !/^\d{0,2}$/.test(value)) return; // chiffres seulement, 2 max
    setForm((prev) => {
      const nextAges = [...prev.childrenAgesArr];
      nextAges[index] = value;
      return { ...prev, childrenAgesArr: nextAges };
    });
  };

  const handleSubmit = async () => {
    if (!(form.firstName.trim() && form.lastName.trim() && form.email.trim() && form.password.length >= 8)) return;
    setLoading(true);
    setErrorMsg("");

    try {
      const session = await supabaseSignUp(form.email.trim(), form.password, {
        first_name: form.firstName.trim(),
        last_name: form.lastName.trim(),
        mother_age: form.motherAge,
        children: form.children,
        children_ages: form.childrenAgesArr.filter((a) => a !== "").join(", "),
        due_date: form.dueDate,
        pregnancy_profile: form.pregnancyProfile,
        newsletter: form.newsletter,
      });

      setLoading(false);
      onComplete(form, session);
    } catch (err) {
  ...
  const L = lang === "fr"
    ? {
        title: "Bienvenue dans Me My Baby", subtitle: "Créez votre profil gratuit pour commencer à explorer l'application.",
        firstName: "Prénom", lastName: "Nom", email: "Courriel", password: "Mot de passe", passwordHint: "8 caractères minimum",
        motherAge: "Votre âge", children: "Nombre d'enfants",
        childAgeLabel: "Âge enfant", due: "Date prévue d'accouchement (si applicable)",
        pregnancyProfileLabel: "Votre grossesse (optionnel)",
        pregnancyProfileOptions: [
          { key: "jumeaux", label: "Grossesse gémellaire" },
          { key: "35plus", label: "35 ans et plus" },
          { key: "risque", label: "Grossesse à risque" },
        ],
        newsletter: "J'accepte de recevoir l'infolettre mensuelle Me My Baby par courriel.",
        submit: "Créer mon profil et continuer",
        note: "La création d'un profil est requise pour utiliser l'application. Vous pourrez ensuite explorer librement — le contenu détaillé est réservé aux membres.",
    ...
    : lang === "es"
    ? {
        title: "Bienvenida a Me My Baby", subtitle: "Crea tu perfil gratuito para empezar a explorar la aplicación.",
        firstName: "Nombre", lastName: "Apellido", email: "Correo electrónico", password: "Contraseña", passwordHint: "Mínimo 8 caracteres",
        motherAge: "Tu edad", children: "Número de hijos",
        childAgeLabel: "Edad del hijo/a", due: "Fecha prevista de parto (si aplica)",
        pregnancyProfileLabel: "Tu embarazo (opcional)",
        pregnancyProfileOptions: [
          { key: "jumeaux", label: "Embarazo múltiple" },
          { key: "35plus", label: "35 años o más" },
          { key: "risque", label: "Embarazo de alto riesgo" },
        ],
        newsletter: "Acepto recibir el boletín mensual de Me My Baby por correo electrónico.",
        submit: "Crear mi perfil y continuar",
        note: "Se requiere crear un perfil para usar la aplicación. Después podrás explorar libremente — el contenido detallado está reservado para miembros.",
    ...
    : {
        title: "Welcome to Me My Baby", subtitle: "Create your free profile to start exploring the app.",
        firstName: "First name", lastName: "Last name", email: "Email", password: "Password", passwordHint: "8 characters minimum",
        motherAge: "Your age", children: "Number of children",
        childAgeLabel: "Child age", due: "Expected due date (if applicable)",
        pregnancyProfileLabel: "Your pregnancy (optional)",
        pregnancyProfileOptions: [
          { key: "jumeaux", label: "Twin pregnancy" },
          { key: "35plus", label: "35 or older" },
          { key: "risque", label: "High-risk pregnancy" },
        ],
        newsletter: "I agree to receive the monthly Me My Baby newsletter by email.",
        submit: "Create my profile and continue",
        note: "Creating a profile is required to use the app. After that you can browse freely — detailed content is reserved for members.",
    ...
    <div style={{
      ...
      <div style={{ width: "100%", maxWidth: 440 }}>
        ...
        <Card>
          ...
          <div>
            <div style={{ display: "flex", gap: 14, flexWrap: "wrap" }}>
              <label style={{ ...labelStyle, flex: 1, minWidth: 140 }}>{L.firstName}
                <input required style={inputStyle} value={form.firstName} onChange={(e) => setForm({ ...form, firstName: e.target.value })} />
              </label>
              <label style={{ ...labelStyle, flex: 1, minWidth: 140 }}>{L.lastName}
            ...
            <div style={{ display: "flex", gap: 14, flexWrap: "wrap" }}>
              <label style={{ ...labelStyle, flex: 1, minWidth: 140 }}>{L.children}
                <input type="number" min={0} max={20} style={inputStyle} value={form.children} onChange={(e) => handleChildrenChange(e.target.value)} />
              </label>
            </div>
            {form.childrenAgesArr.length > 0 && (
              <div style={{ display: "flex", gap: 14, flexWrap: "wrap" }}>
                {form.childrenAgesArr.map((age, i) => (
                  <label key={i} style={{ ...labelStyle, flex: 1, minWidth: 120 }}>{`${L.childAgeLabel} ${i + 1}`}
                    <input
                      inputMode="numeric"
                      pattern="[0-9]*"
                      style={inputStyle}
            ...
            {form.dueDate && (
              <div style={{ marginBottom: 14 }}>
                <p style={{ fontSize: 12.5, color: COLORS.muted, fontWeight: 600, marginBottom: 8 }}>{L.pregnancyProfileLabel}</p>
                <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                  {L.pregnancyProfileOptions.map((opt) => {
                    const active = form.pregnancyProfile.includes(opt.key);
                    return (
                      <button
                        key={opt.key} type="button"
                        onClick={() => setForm((f) => ({
                          ...f,
                          pregnancyProfile: active ? f.pregnancyProfile.filter((k) => k !== opt.key) : [...f.pregnancyProfile, opt.key],
                        }))}
                        style={{
            ...
            <label style={{ display: "flex", gap: 10, alignItems: "flex-start", fontSize: 13, color: COLORS.text, margin: "6px 0 18px", cursor: "pointer" }}>
              <input type="checkbox" checked={form.newsletter} onChange={(e) => setForm({ ...form, newsletter: e.target.checked })} style={{ marginTop: 3 }} />
              <span>{L.newsletter}</span>
            </label>
            {errorMsg && (
        ...
        {mode === "signup" && <p style={{ fontSize: 12, color: COLORS.muted, textAlign: "center", marginTop: 14, lineHeight: 1.5 }}>{L.note}</p>}
        <button
        ...
        <button
          ...
          style={{
            ...
            color: COLORS.muted, fontSize: 11, textDecoration: "underline", cursor: "pointer", opacity: 0.6,
          }}
        >
          {L.designerSkip}
        </button>
...
export default function App() {
  ...
  const mapProfileRow = (row) => ({
    ...
    motherAge: row.mother_age || "", children: row.children ?? "0",
  ...
  // Au chargement de l'app, on tente de restaurer la session déjà ouverte (localStorage)
  useEffect(() => {
  ...
  const handleAuthComplete = async (profileForm, authSession) => {
    if (authSession?.access_token) {
      ...
      try {
        ...
        if (profileRow) { setUserProfile(mapProfileRow(profileRow)); return; }
  ...
  const renderContent = () => {
    switch (active) {
      ...
      case "abonnement": return <AbonnementUnified lang={lang} isMember={isMember} goTo={goTo} onBecomeMember={becomeMember} />;
      case "profil": return <ProfileSection lang={lang} children={children} setChildren={setChildren} userProfile={userProfile} onLogout={logout} session={session} />;
        ...
        <div style={{ maxWidth: 1180, margin: "0 auto", padding: "14px 24px", display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12 }}>
          ...
          <button
            ...
            style={{
              ...
              fontSize: 13, fontWeight: 600, color: COLORS.teal, cursor: "pointer",
              boxShadow: "0 1px 3px rgba(47,72,88,0.06)",
            }}
          >
            <Globe size={15} />
