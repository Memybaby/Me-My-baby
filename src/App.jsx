...
// Les photos prises directement avec l'appareil photo d'un téléphone peuvent peser plusieurs Mo à pleine résolution ;
// en garder plusieurs en mémoire en même temps (ex. photo du parent + de 2-3 enfants) peut saturer la mémoire du navigateur
// et provoquer un rechargement forcé de la page ("manque de mémoire"). On les réduit donc systématiquement avant utilisation.
function resizeImageForUpload(file, maxDim = 640, quality = 0.75) {
  return new Promise((resolve) => {
    if (!file || !file.type?.startsWith("image/")) { resolve({ dataUrl: null, file }); return; }
