...
//
// IMPORTANT : billing_anchor_date correspond à la date du TOUT PREMIER paiement (fin de l'essai
// de 5 jours). Cette première fois-là, la cliente a déjà reçu le courriel de bienvenue qui
// mentionne que le paiement se fera automatiquement après l'essai — il ne faut donc PAS renvoyer
// un rappel à ce moment-là, seulement pour les renouvellements des années suivantes (2e année,
// 3e année, etc.). On retourne donc ici, en plus de la date, un indicateur "isFirstCharge" qui
// dit si la prochaine date de facturation est encore la date d'ancrage elle-même (= premier
// paiement, jamais encore renouvelé) ou si on a déjà avancé d'au moins une année (= vrai
// renouvellement).
function nextAnnualBillingInfo(anchorDateStr) {
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const today = new Date(new Date().toDateString());
    let sent = 0;
    let skippedFirstCharge = 0;
    const errors = [];

    for (const profile of profiles) {
      if (!profile.email || !profile.billing_anchor_date) continue;
      const info = nextAnnualBillingInfo(profile.billing_anchor_date);
      if (!info) continue;

      // Le tout premier paiement (fin de l'essai de 5 jours) est déjà annoncé dans le courriel
      // de bienvenue — on n'envoie donc PAS de rappel pour celui-ci, seulement pour les
      // renouvellements des années suivantes.
      if (info.isFirstCharge) {
        skippedFirstCharge += 1;
        continue;
      }

      const nextDate = info.date;
    ...
    res.status(200).json({ ok: true, checked: profiles.length, sent, skippedFirstCharge, errors });
  } catch (e) {
