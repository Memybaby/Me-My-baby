...
export default async function handler(req, res) {
  ...
  try {
    ...
    const profilesRes = await fetch(
      `${SUPABASE_URL}/rest/v1/profiles?email=not.is.null&subscription_status=neq.cancelled&newsletter=neq.false&select=id,email,first_name,due_date,language`,
    ...
    for (const profile of profiles) {
      ...
      const { subject: finalSubject, html } = buildEmail(lang, profile.first_name, subject, mainHtml, suggestion, extraChildren, `${APP_URL}/api/unsubscribe?uid=${profile.id}&lang=${lang}`);
